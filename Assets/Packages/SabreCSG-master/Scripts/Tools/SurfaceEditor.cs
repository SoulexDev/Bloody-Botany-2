...
namespace Sabresaurus.SabreCSG
{
    public class SurfaceEditor : Tool
    {
        ...
        private bool undoRecorded = false; // Used so that when translating or rotating undo is only recorded at the start, not per frame
        ...
        private Vector2 appliedDelta;

        // Scale
        private string scaleAmount = "1";

        private string uScaleString = null;
        private string vScaleString = null;

        private string uOffsetString = null;
        private string vOffsetString = null;
        ...
        private List<Polygon> previousHits = new List<Polygon>();
        ...
        private bool copyMaterialHeld = false;

        private VertexColorWindow vertexColorWindow = null;

        // Whether we are actively searching for hidden faces.
        private bool findingHiddenFaces = false;

        // Whether quick select is selecting or deselecting polygons.
        private enum QuickSelectModes
        { Additive, Subtractive };

        private QuickSelectModes QuickSelectMode = QuickSelectModes.Additive;
        ...
        private void EnsureCurrentPolygonSelected()
        {
        ...
        private void OnMouseDragQuickSelect(SceneView sceneView, Event e)
        {
            if (!EditorHelper.IsMousePositionInIMGUIRect(e.mousePosition, ToolbarRect))
            {
                Ray ray = HandleUtility.GUIPointToWorldRay(Event.current.mousePosition);
        ...
        private void OnMouseDragFollowLastFace(SceneView sceneView, Event e)
        {
            if (!EditorHelper.IsMousePositionInIMGUIRect(e.mousePosition, ToolbarRect))
            {
                Ray ray = HandleUtility.GUIPointToWorldRay(Event.current.mousePosition);
        ...
        private void FollowLastFace(Polygon sourceTargetPolygon)
        {
        ...
        private float FindAngle(Vertex vertex1, Vertex vertex2, Vertex vertex3, Plane polygonPlane)
        {
        ...
        private void OnDragPerform(SceneView sceneView, Event e)
        {
        ...
        private void TransformUVs(UVUtility.UVTransformation transformationMethod, UVUtility.TransformData transformData, bool recordUndo)
        {
            for (int polygonIndex = 0; polygonIndex < selectedSourcePolygons.Count; polygonIndex++)
            {
                Polygon polygon = selectedSourcePolygons[polygonIndex];
        ...
        private void TransformUVs(Polygon polygon,
        ...
        private void RotateAroundCenter(float rotationAmount, bool recordUndo)
        {
            for (int polygonIndex = 0; polygonIndex < selectedSourcePolygons.Count; polygonIndex++)
            {
                Polygon polygon = selectedSourcePolygons[polygonIndex];
        ...
        private void DrawManualTextBoxes()
        {
            float? northScale = 1;
            float? eastScale = 1;
        ...
        private void OnToolbarGUI(int windowID)
        {
            ...
            if (selectHelpersVisible)
            {
                ...
                if (GUILayout.Button("Same Brush", EditorStyles.miniButton))
                {
                    SelectSameBrush();
                }
        ...
        private void SelectSameMaterial()
        {
        ...
        private void SelectSameBrush()
        {
            ...
            {
                selectedSourcePolygons.Add(item.Key);
                matchedBrushes.Add(item.Key, item.Value);
            }
        }

        private void SelectAdjacentWalls()
        {
        ...
        private void SelectAdjacentFloors()
        {
            AdjacencyFilters.MatchMaterial filter = null;
            if (limitToSameMaterial)
            {
        ...
        private void SelectAdjacentCeilings()
        {
            AdjacencyFilters.MatchMaterial filter = null;
            if (limitToSameMaterial)
            {
        ...
        private void SelectAdjacentCoplanar()
        {
            AdjacencyFilters.MatchMaterial filter = null;
            if (limitToSameMaterial)
            {
        ...
        private void SelectAdjacentAll()
        {
            AdjacencyFilters.MatchMaterial filter = null;
            if (limitToSameMaterial)
            {
        ...
        private void SetSelectionFromPolygonIDs(List<int> polygonIDs)
        {
            ResetSelection();
        ...
        /// </summary>
        private void ExtrudeBrushesFromSelection()
        {
        ...
        private void AutoUV(bool useWorldSpace)
        {
            for (int polygonIndex = 0; polygonIndex < selectedSourcePolygons.Count; polygonIndex++)
            {
                Polygon polygon = selectedSourcePolygons[polygonIndex];
        ...
        private void PlanarMap(Vector3 planarDirection)
        {
            for (int polygonIndex = 0; polygonIndex < selectedSourcePolygons.Count; polygonIndex++)
            {
                Polygon polygon = selectedSourcePolygons[polygonIndex];
        ...
        private void Align(AlignDirection direction)
        {
            for (int polygonIndex = 0; polygonIndex < selectedSourcePolygons.Count; polygonIndex++)
            {
                Polygon polygon = selectedSourcePolygons[polygonIndex];
        ...
        private void UserExcludePolygon(Polygon sourcePolygon)
        {
        ...
        private void UserIncludePolygon(Polygon sourcePolygon)
        {
            Polygon[] builtRenderPolygons = csgModel.BuiltPolygonsByIndex(sourcePolygon.UniqueIndex);
